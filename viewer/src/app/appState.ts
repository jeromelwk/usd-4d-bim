...
export const state = {
  ...
  // Patched for USD 4D BIM: full multi-selection, in viewport click order.
  // `selectedPrimPath` is kept in sync (single entry, or null when 0/2+ are
  // selected) for the existing single-prim UI (attributes panel, "F" frame
  // shortcut, stageEdits re-selection) that predates multi-select.
  viewportSelection: [] as string[],
};
