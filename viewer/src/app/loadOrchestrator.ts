...
export async function loadFiles(files: File[], rootFileHint?: string): Promise<void> {
  if (!files.length) {
    return;
  }
  ...
  // Patched for USD 4D BIM: honor an explicit root file hint (e.g. from an
  // automation manifest) instead of always guessing among same-depth files —
  // needed when a sublayer/root pair (schedule.usda + geometry.usda) can't be
  // told apart by the heuristic alone.
  const rootFile = (rootFileHint && files.find((f) => (f.webkitRelativePath || f.name) === rootFileHint))
    || pickLikelyRootFile(files);
  ...
  if (renderables.length > 0) {
    ...
    if (state.currentStageSummary?.environment && state.lightingMode === "hdri") {
      state.viewport.setHdriIntensity(state.hdriIntensity);
      state.viewport.setHdriRotation(state.currentStageSummary.environment.rotation ?? 0);
      state.viewport.setHdriMapVisible(state.hdriMapVisible);
    }
    state.currentRendererStats = collectRendererStats(renderables, gaussianSplats);
    renderStageSummary(state.currentStageSummary);
...
export async function loadAutomationManifestStage(manifestUrl: string): Promise<void> {
  ...
  await loadFiles(files, manifest.rootFile);
